...
contract ERC721 is IERC721, IERC721Metadata, IERC721Enumerable, IERC165 {
    ...
    function tokenByIndex(uint256 index) external view onlyValidTokenId(index) returns (uint256) {
        return index;
    }

    function tokenOfOwnerByIndex(address owner, uint256 index) external view returns (uint256) {
        require(index < ownerToTokenIds[owner].length, "Index out of owner's balance bounds.");
    ...
    function mint(string memory tokenUri) external {
        ownerToTokenIds[msg.sender].push(nextTokenId);
    ...
    function _transfer(address from, address to, uint256 tokenId) private {
        ...
        for(uint256 i = 0; i < ownerToTokenIds[from].length; i++) {
            if (ownerToTokenIds[from][i] == tokenId) {
                ownerToTokenIds[from][i] = ownerToTokenIds[from][ownerToTokenIds[from].length-1];
